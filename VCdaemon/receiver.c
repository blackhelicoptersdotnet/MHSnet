...
void		PassInPkt _FA_((int, int, int, Ulong, Uchar *));
void		RecvData _FA_((int, int, int, Ulong, Uchar *));
void		Recv_EOM _FA_((int, int, int, Ulong, Uchar *));
void		RecvREOT _FA_((int, int, int, Ulong, Uchar *));
void		RecvRMQE _FA_((int, int, int, Ulong, Uchar *));
void		Recv_SOM _FA_((int, int, int, Ulong, Uchar *));
...
void
PassInPkt(chan, type, size, addr, data)
	int		chan;
	int		type;
	int		size;
	Ulong		addr;
	Uchar *		data;
{
	Trace4(1, "PassInPkt type %d size %d on channel %d", type, size, chan);
...
void
RecvData(chan, type, size, addr, data)
	int		chan;
	int		type;
	register int	size;
	register Ulong	addr;
...
void
Recv_EOM(chan, type, size, addr, data)
	int		chan;
	int		type;
	int		size;
	Ulong		addr;
...
void
RecvREOT(chan, type, size, addr, data)
	int		chan;
	int		type;
	int		size;
	Ulong		addr;
...
void
RecvRMQE(chan, type, size, addr, data)
	int		chan;
	int		type;
	int		size;
	Ulong		addr;
	Uchar *		data;
{
	...
	)
	{
		...
		*/
		...
		if ( chan == IDLE )
		{
			...
			if ( addr != 0 )		/* Transmitter can adjust params */
			{
				OldDaemon = false;
			...
			OldDaemon = true;		/* Old daemon */
			IDLETime = 0;			/* No idle timeouts */
		}
	...
	{
		if ( OldDaemon )
			return;		/* Our IDLE came back from old daemon */

		/*
		**	IDLE echoed on link!
		*/

		AbortInMesgs(english("IN aborted by IDLE ECHO"));	/* Close off any open messages. */
...
void
Recv_SOM(chan, type, size, addr, data)
	int		chan;
	int		type;
	int		size;
	...
	Uchar *		data;
{
	...
	Trace7(1, "Recv_SOM on channel %d, id=%.*s, size=%d, link=%.*s.", chan, MSGIDLENGTH, A_SOMID(data), addr, size - SOMMDATASIZE, A_SOMLINK(data));
...
#	endif
	...
	if
	(
		LinkAdLength == (size - SOMMDATASIZE)
		&&
		strnccmp(LinkAddress, A_SOMLINK(data), LinkAdLength) == STREQUAL
	)
	{
		/*
		**	Message echoed on link!
		*/

		AbortInMesgs(english("IN aborted by SOM ECHO"));	/* Close off any open messages. */
	...
	if
	(
		HomeAdLength != (size - SOMMDATASIZE)
		||
		strnccmp(HomeAddress, A_SOMLINK(data), HomeAdLength) != STREQUAL
	)
	{
		/*
		**	Other end doesn't know who we are.
		*/
