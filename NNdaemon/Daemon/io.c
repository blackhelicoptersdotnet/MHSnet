...
Unlink(file)
	char *	file;
{
	if ( unlink(file) == SYSERROR )
		Syserror(CouldNot, UnlinkStr, file);
...
/*
**	write(2), taking care of partial writes, with error recovery.
*/

int
WriteJ(chan, fd, addr, size, file)
	int		chan;
	int		fd;
	char *		addr;
	int		size;
	char *		file;
{
	...
	Trace5(4, "WriteJ chan %d fd %d size %d file \"%s\"", chan, fd, s, file);
